...
class Test_load_cube(IrisTest):
    ...
    def test_ordering_for_realization_threshold_percentile_coordinate(self):
        ...
        cube = add_coordinate(
            cube, [0, 1, 2], "realization", dtype=np.int32, coord_units="1"
        )
        cube = add_coordinate(
            cube, [10, 50, 90], "percentile", dtype=np.float32, coord_units="%"
        )
...
class Test_load_cubelist(IrisTest):
    ...
    def setUp(self):
        ...
        self.med_cloud_filepath = os.path.join(self.directory, "medium_cloud.nc")
    ...
    def test_no_partial_merge_single_arg(self):
        ...
        low_cloud_cube.rename("low_type_cloud_area_fraction")
        low_cloud_cube.units = 1
        low_cloud_cube.coord("time").points = low_cloud_cube.coord("time").points + 3600
