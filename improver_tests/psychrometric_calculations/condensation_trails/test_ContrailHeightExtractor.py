...
def make_cube(data, name, units, dims=('pressure', 'latitude', 'longitude')):
    ...
    for dim, (coord_name, points) in enumerate(coords):
        if coord_name == 'engine_factor':
            cube.add_dim_coord(iris.coords.DimCoord(points, long_name='engine_factor'), dim)
...
def test_max_extraction():
    ...
    formation = np.array([[[[1, 1], [2, 2]], [[1, 1], [2, 0]]],  # engine_factor 0
                          [[[2, 0], [1, 1]], [[0, 1], [2, 1]]]])  # engine_factor 1
    # --> height data (pressure (x2), latitude (x2), longitude (x2))
    height = np.array([[[100, 200], [300, 400]], [[200, 300], [400, 500]]])  # pressure level 1, pressure level 2

    # 2. Turn test data into test cubes ready for passing into ContrailHeightExtractor
    formation_cube = make_cube(formation, 'formation', '1', dims=('engine_factor', 'pressure', 'latitude', 'longitude'))
    height_cube = make_cube(height, 'height', 'm', dims=('pressure', 'latitude', 'longitude'))
    ...
    expected_non_persistent_max_height = np.array([[[200, 300], [np.nan, np.nan]], [[np.nan, 300], [300, 500]]])
    expected_persistent_max_height = np.array([[[np.nan, np.nan], [400, 400]], [[100, np.nan], [400, np.nan]]])
    ...
    assert np.allclose(non_persistent.data, expected_non_persistent_max_height, equal_nan=True)
...
def test_min_extraction():
    ...
    # Expected results for *max* heights of contrail formation associated with formation and height data above.
    # Result data arrays have the same dimentions (engine_factor (x2), latitude (x2), longitude (x2))
    expected_non_persistent_min_height = np.array([[[100, 200], [np.nan, np.nan]], [[np.nan, 300], [300, 400]]])
    expected_persistent_min_height = np.array([[[np.nan, np.nan], [300, 400]], [[100, np.nan], [400, np.nan]]])
    ...
    # 5. Assert that the results from the extractor match the expected results
    assert np.allclose(non_persistent.data, expected_non_persistent_min_height, equal_nan=True)
...
def test_error_handling():
    ...
    height = np.array([[[100, 200, 300], [300, 400, 500]], [[100, 200, 300], [300, 400, 500]]])  # Height has more points
