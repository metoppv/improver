...
class SetupExpectedCoefficients(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.expected_mean_pred_each_grid_point = {
            ...
            "emos_coefficient_beta": np.array(
                [[0.9999, 1.0001, 1.0006], [1.0008, 1.0003, 1.0], [1.0001, 1.0006, 1.0]]
            ),
        ...
        self.expected_realizations_pred_minimise_each_grid_point = {
            ...
            "emos_coefficient_delta": np.array(
                [[-0.0, 0.0, -0.0005], [0.0009, 0.0, 0.0003], [0.0001, -0.0008, 0.0001]]
            ),
        }
        ...
        self.expected_realizations_each_site_alt = {
            ...
            "emos_coefficient_beta": np.array(
                [
                    [
                        [0.7506, 0.7514, 0.7526, 0.7505],
                        [0.6633, 0.6625, 0.6611, 0.6636],
                    ],
                    [
                        [0.0414, 0.0207, 0.0138, 0.0104],
                        [0.0464, 0.0233, 0.0156, 0.0116],
                    ],
                ]
            ),
...
class Test_create_coefficients_cubelist(SetupCubes, SetupExpectedCoefficients):
    ...
    def test_coefficients_from_mean(self):
        ...
        for cube in result:
            ...
            self.assertEqual(cube.coord("forecast_period"), self.expected_fp)
            ...
            self.assertDictEqual(dict(cube.attributes), self.attributes)
    ...
    def test_attributes_for_truncnorm(self):
        ...
        for cube in result:
            assert set(cube.attributes.keys()) == set(self.attributes.keys())
            for key in self.attributes.keys():
                try:
                    assert cube.attributes[key] == self.attributes[key]
                except ValueError:
                    np.testing.assert_array_equal(
                        cube.attributes[key], self.attributes[key]
                    )
    ...
    def test_coefficients_point_by_point_sites(self):
        ...
        for cube in result:
            ...
            self.assertAlmostEqual(
                cube.coord("latitude"),
                self.historic_forecast_spot_cube.coord("latitude"),
            )
            self.assertAlmostEqual(
                cube.coord("longitude"),
                self.historic_forecast_spot_cube.coord("longitude"),
            )
            self.assertAlmostEqual(
                cube.coord("altitude"),
                self.historic_forecast_spot_cube.coord("altitude"),
            )
            self.assertAlmostEqual(
                cube.coord("wmo_id"), self.historic_forecast_spot_cube.coord("wmo_id")
            )
            self.assertDictEqual(dict(cube.attributes), self.attributes)
    ...
    def test_point_by_point_additional_predictor(self):
        ...
        and metadata, is returned when the ensemble mean is used as the predictor
        when estimating coefficients for each site independently."""
        expected_predictor_index_coord = DimCoord(
            np.array([0, 1], np.float32), units="1", long_name="predictor_index"
        )
        expected_predictor_name_coord = AuxCoord(
            np.array(["air_temperature", "altitude"]),
            units="no_unit",
...
class Test_compute_initial_guess(unittest.TestCase):
    ...
    def test_realizations_predictor_estimate_coefficients(self):
        ...
        result = plugin.compute_initial_guess(
            self.truth.data, forecast_predictor.data, predictor, self.no_of_realizations
        )
        np.testing.assert_array_almost_equal(
    ...
    def test_mean_predictor_multiple_forecast_predictors_default(self):
        ...
        self.assertIsInstance(result, np.ndarray)
        np.testing.assert_array_almost_equal(
            result, self.expected_multiple_predictors_default
        )
...
class Test_mask_cube(SetupCubes):
    ...
    def test_basic_2D_input_cube(self):
        ...
        np.testing.assert_array_almost_equal(expected_result.data, cube2D.data.data)
        np.testing.assert_array_equal(
...
class Test_process(
    ...
    def test_point_by_point_default_initial_guess(self):
        ...
        for cube in result:
            self.assertEMOSCoefficientsAlmostEqual(
                cube.data, self.expected_mean_pred_minimise_each_grid_point[cube.name()]
            )
            self.assertIn(cube.name(), self.expected_coeff_names)
            self.assertEqual(
                [c.name() for c in cube.coords(dim_coords=True)], expected_dim_coords
