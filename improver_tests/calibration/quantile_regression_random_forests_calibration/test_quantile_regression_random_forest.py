...
def _create_forecasts(
    ...
    day_of_training_period_coord = AuxCoord(
        np.int32(day_of_training_period), long_name="day_of_training_period", units="1"
    )
    cube.add_aux_coord(day_of_training_period_coord)
    return cube
...
def _run_train_qrf(
    ...
    truth_data=[[4.2, 4.2], [3.8, 3.8], [4, 4], [4.1, 4.1]],
):
    forecast_cubes = CubeList()
    for frt, vt, day in zip(
        forecast_reference_times, validity_times, day_of_training_period
    ):
        forecast_cube = _create_forecasts(frt, vt, realization_data, day)
        forecast_cubes.append(forecast_cube)
    forecast_cube = forecast_cubes.merge_cube()

    # truth_cube = forecast_cube.copy()
...
def test_prep_feature_single_time(feature, expected, expected_dtype):
    ...
    forecast_cube = _create_forecasts(
        forecast_reference_time, validity_time, data, day_of_training_period
    )
...
def test_prep_feature_multiple_times(feature, expected, expected_dtype):
    ...
    for frt, vt, day in zip(
        ...
        forecast_cubes.append(_create_forecasts(frt, vt, data, day))
    forecast_cube = forecast_cubes.merge_cube()
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (1, 1, 73, 5, None, 0, {}, False, 4.2),  # Different random state
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (1, 1, 55, 5, None, 0, {}, False, 4.1),  # Fewer estimators and reduced depth
        (1, 1, 73, 5, None, 0, {}, False, 4.0),  # Different random state
        (2, 2, 55, 5, "log", 10, {}, False, 2.650),  # Log transformation
        (2, 2, 55, 5, "log10", 10, {}, False, 1.151),  # Log10 transformation
        (2, 2, 55, 5, "sqrt", 10, {}, False, 3.762),  # Square root transformation
        (2, 2, 55, 5, "cbrt", 10, {}, False, 2.419),  # Cube root transformation
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ({"wind_speed_at_10m": ["latitude"]}, [55], False, [4.15]),  # noqa Without the target
        ({"wind_speed_at_10m": ["mean"]}, [5], True, [4.15]),  # With static data
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ([0.5], None, 0, False, [4.15, 4.05]),  # One quantile
        ([0.1, 0.5, 0.9], None, 0, False, [[4.11, 4.01], [4.15, 4.05], [4.19, 4.09]]),  # noqa Multiple quantiles
        ([0.1, 0.5, 0.9], "log", 10, False, [[2.65, 2.64], [2.65, 2.64], [2.65, 2.65]]),  # noqa Log transformation
        (
            [0.1, 0.5, 0.9],
            "log10",
            10,
            False,
            [[1.15, 1.14], [1.15, 1.14], [1.15, 1.15]],
        ...
        (
            ...
            10,
            False,
            [[2.42, 2.41], [2.42, 2.41], [2.42, 2.42]],
        ),  # Cube root transformation
        ([0.1, 0.5, 0.9], None, 0, True, [[4.11, 4.01], [4.15, 4.05], [4.19, 4.09]]),  # noqa Include static data
    ],
)
def test_apply_qrf(
    tmp_path,
    ...
    forecast_cube = _create_forecasts(frt, vt, data, day_of_training_period)
    ...
    result = plugin.process(
        forecast_cube, forecast_cube, qrf_model, feature_cubes=feature_cubes
    )
