...
class Test_process(unittest.TestCase):
    ...
    def test_handles_different_sized_arrays(self):
        ...
        msg = f"Input arrays must have identical sizes, but sizes are: wspeed_at_h_ref={np.size(u_href)}, h_ref={np.size(self.h_ref)}, roughness_length_z0={np.size(self.z_0)}, ustar_mask={np.size(self.mask)}"
