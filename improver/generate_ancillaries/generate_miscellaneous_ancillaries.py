...
def generate_distance_to_ocean(
    ...
    """Generate a distance to ocean ancillary cube. The DistanceTo plugin can't be used directly
    because there isn't a GeoDataFrame for the ocean.

    The DistanceTo class is used with the coastline GeoDataframe to calculate the distance of each site to thae coastline.
    The DistanceTo class is also used to calcualt the distance to land for each site. This identifies which sites are
    land and which are ocean. Sites in the ocean can then be sem to 0m as they are in the ocean.
    ...
    Args:
        ...
        site_cube:
            A cube containing the site locations. There must be latitude and longitude coordinates.
    Returns:
...
def generate_distance_to_water(distance_to_water_feature: CubeList) -> Cube:
    """Generate a distance to water ancillary cube. The distance to water is the minimum of all the
    provided distance to water features, such as rivers, lakes, and oceans.

    The first cube in distance_to_water_feature is used as a template for the output metadata with
    the name updated to "distance_to_water".

    Args:
        distance_to_water_feature:
            A CubeList containing distance to water features from sites (i.e. rivers, lakes, and oceans).
    ...
    distances = [cube.data for cube in distance_to_water_feature]
...
def generate_land_area_fraction_at_sites(
    ...
    The Corine Land cover is available from https://doi.org/10.2909/960998c1-1870-4e82-8051-6485205ebbac. This
    function requires the land cover file is provided as an iris cube.

    The land_cover cube is first regridded be on the same projection as the provided gridded template.
    A neighbourhood is then applied to the regridded land cover cube to calculate the land area fraction
    at each grid point. The neighbourhood radius is set to 2500m. The land area fraction is then extracted
    at the site locations using the neighbour_cube.

    Args:
        land_cover_cube:
            A cube containing the Corine Land cover data. The data values should be integers representing
            different land cover types.
        gridded_template_cube:
            A cube containing the gridded template to regrid the land cover cube to.
        neighbour_cube:
            A cube containing information about the spot data sites and their grid point neighbours.

    Returns:
        A cube containing the land area fraction at the site locations.
    """

    # 41-44 is key for water in the Corine Land cover dataset.
    ...
    # 48 is key for complex land surfaces in Corine
    land_mask.data = np.where(land_cover_cube.data == 48, 1, land_mask.data)

    # regrid the land mask to the same projection as the gridded template. This may lead to grid squares
    # not being 1 or 0 but as the land mask will be neighbourhooded this will not be an issue.
    ...
    cube_attributes = spot_extracted_land_fraction.attributes.copy()
    for attribute in cube_attributes:
        if attribute != "conventions":
            del spot_extracted_land_fraction.attributes[attribute]
