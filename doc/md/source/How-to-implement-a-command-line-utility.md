...
`improver/cli` that uses [clize](https://github.com/epsy/clize) to
...
[Iris](https://github.com/SciTools/iris) cubes. When run from the
...
Plugins are Python classes with unit tests. See the
`Code-Style-Guide`{.interpreted-text role="doc"} section on plugins for
more information.
...
See the `Running-at-your-site`{.interpreted-text role="doc"} page for
information on how to run the acceptance tests.
...
(`IMPROVER_ACC_TEST_DIR`, see `Running-at-your-site`{.interpreted-text
role="doc"}). It is also possible to use this method to recreate all
KGOs following changes that may affect them, for example modifications
to metadata.
...
checksums listed in
[improver_tests/acceptance/SHA256SUMS](https://github.com/metoppv/improver/blob/master/improver_tests/acceptance/SHA256SUMS).
...
Running an individual test, e.g. `pytest -m acc -k threshold` will also
compare the input data against the expected checksums and report if
there is a mismatch.
