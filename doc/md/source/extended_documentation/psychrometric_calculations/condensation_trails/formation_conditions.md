...
The local vapour pressure sits above the tangent to the saturated vapour
pressure curve,

$$C_1 = e_{local} - mT > I_{critical} \ \ \text{,}$$

where $e_{local}$ is the local vapour pressure, $m$ is the engine mixing
ratio, $T$ is the ambient air temperature, and $I_{critical}$ is the
critical intercept.
